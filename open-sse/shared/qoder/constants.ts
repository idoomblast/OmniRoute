/**
 * Qoder API constants ported from 9router (decolua/9router, MIT) — itself
 * ported from CLIProxyAPIPlus qoder-provider branch (internal/auth/qoder).
 *
 * Endpoint set:
 *   openapi.qoder.sh   - device flow + userinfo + quota usage + PAT job-token exchange
 ...
 *   api2.qoder.sh      - alternate inference host: the official qodercli serves
 *                        job-token (jt-...) traffic from api2 because api3
 *                        rejects it with "Login expired" (403)
...
// Inference endpoints (under /algo on api3/api2.qoder.sh, all COSY-signed)
...
/**
 * Canonical Qoder model identifiers served by /algo/api/v2/model/list.
 * Note: the live catalog (see qoderModels.ts) is authoritative — a request
 * carries the exact server-published model_config for the chosen key, and
 * sending a stale/wrong block silently downgrades to a different model.
 */
 ...
 * Matches the CLIProxyAPIPlus branch and live qodercli traffic — and the
 * key already embedded in qoderCli.ts (buildCosyHeadersForValidation).
 */
