/**
 * Qoder COSY (hybrid RSA+AES+MD5) signing, ported from 9router
 * (decolua/9router, MIT) — itself ported from CLIProxyAPIPlus
 ...
 *
 * Note: OmniRoute's older buildCosyHeadersForValidation (qoderCli.ts) shipped
 * only 5 of these headers and failed upstream — the full header set + body
 * hash/length + WAF-encoded body is what makes the signature accepted.
 */
 ...
 * @param creds  userId + authToken (device `dt-` or job `jt-` token) +
 *   optional display name/email/persisted machineId.
